...
class Artist_Guest:
	def __init__(self, staff_dict):
		...
		if self.lobster == "":
			self.lobster = set_field_if_exists('Would you like to add on the Wednesday night lobster dinner?', staff_dict)
	...
	def to_row(self):
		...
		# Meals / Dietary Reqs / Other Reqs / Lobster / Meals Waitlist / City / State / Photo optout / Parking / Income
		self.meals, self.meal_reqs, self.meal_other_reqs, self.lobster, None, self.city, self.state, self.photo_optout, None, self.income,
